...
public class AdvancedModelBox extends BasicModelPart {
    ...
    public void render(MatrixStack matrixStackIn, VertexConsumer bufferIn, int packedLightIn, int packedOverlayIn, int color) {
        if (this.showModel) {
            if (!this.cubeList.isEmpty() || !this.childModels.isEmpty()) {
                ...
                if(!scaleChildren){
                    matrixStackIn.scale(1F / Math.max(this.xScale, 0.0001F), 1F / Math.max(this.yScale, 0.0001F) , 1F / Math.max(this.zScale, 0.0001F));
                }
                while (var9.hasNext()) {
                    BasicModelPart lvt_10_1_ = (BasicModelPart) var9.next();
                    lvt_10_1_.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, color);
                }
    ...
    public Vec3d getWorldPos(Entity entity, float delta) {
        ...
        matrixStack.multiply(quatFromRotationXYZ(0, -dYaw + 180, 0, true));
