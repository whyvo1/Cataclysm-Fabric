...
public class DynamicChain {
    ...
    public void updateBendConstraint(float gravityAmount, float stiffness, float stiffnessFalloff, float damping, int numUpdates, boolean useFloor) {
        ...
        for (int j = 0; j < numUpdates; j++) {
            for (int i = 0; i < p.length - 1; i++) {
                ...
                T[i] = wrapAngles(r[i].subtract(target)).multiply(-stiffness/(Math.pow(i + 1, stiffnessFalloff)));
                ...
                ra[i] = T[i].multiply(1 / (m[i + 1] * d[i + 1] * d[i + 1]));
                rv[i] = rv[i].add(ra[i].multiply(1/((float)numUpdates))).multiply(damping);
                rv[i] = wrapAngles(rv[i]);
                r[i] = r[i].add(rv[i].multiply(1/((float)numUpdates)));
                r[i] = wrapAngles(r[i]);

                p[i + 1] = fromPitchYaw((float)(r[i].y - Math.PI/2), (float)(r[i].x - Math.PI/2)).multiply(d[i + 1]).add(p[i]);
    ...
    public void updateSpringConstraint(float gravityAmount, float dampAmount, float stiffness, float maxForce, boolean doAttract, float attractFalloff, int numUpdates) {
        for (int j = 0; j < numUpdates; j++) {
            for (int i = 0; i < p.length; i++) {
                a[i] = F[i].multiply(1 / m[i]);
                v[i] = v[i].add(a[i].multiply(1/((float)numUpdates)));
                p[i] = p[i].add(v[i].multiply(1/((float)numUpdates)));
                ...
                disp = disp.normalize().multiply(disp.length() - d[i]);
                Vec3d damp = v[i].multiply(dampAmount);
                ...
                F[i] = disp.multiply(-stiffness * (disp.length())).add(gravity.multiply(m[i])).subtract(damp);
                if (i == 0 || doAttract) {
                    F[i] = F[i].add(attract.multiply(1 / (1 + i * i * attractFalloff)));
                }
                if (F[i].length() > maxForce) F[i].normalize().multiply(maxForce);
...
//            }
    ...
    public void updateChain(float delta, AdvancedModelBox[] chainOrig, AdvancedModelBox[] chainDynamic, float gravityAmount, float stiffness, float stiffnessFalloff, float damping, int numUpdates, boolean useFloor) {
        ...
        for (int i = chainDynamic.length - 1; i >= 0; i--) {
            ...
            Vec3d renderPos = p[i].add(v[i].multiply(delta)).add(a[i].multiply(0.5 * delta * delta));
...
//                dynModelRenderers[i].setScale(1, 1, 1 + (float)diffRotated.z/16);
    ...
    public void render(MatrixStack matrixStackIn, VertexConsumer bufferIn, int packedLightIn, int packedOverlayIn, float red, float green, float blue, float alpha, AdvancedModelBox[] dynModelRenderers) {
