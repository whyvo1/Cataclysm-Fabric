...
public class CMMathUtil {
    public static float approachSmooth(float current, float previous, float desired, float desiredSpeed, float deltaSpeed) {
        ...
        float speed = MathHelper.stepTowards(prevSpeed, desiredSpeed, deltaSpeed);
    ...
    public static float approachDegreesSmooth(float current, float previous, float desired, float desiredSpeed, float deltaSpeed) {
        float desiredDifference = MathHelper.subtractAngles(current, desired);
        float previousDifference = MathHelper.subtractAngles(current, previous);
    ...
    public static Vec3d getOffsetPos(Entity entity, double offsetX, double offsetY, double offsetZ, float rotationX, float rotationY) {
        Vec3d Vec3 = (new Vec3d(offsetZ, offsetY, offsetX)).rotateZ(rotationX * 0.017453292F).rotateY(-rotationY * 0.017453292F - 1.5707964F);
        return entity.getPos().add(Vec3.x, Vec3.y, Vec3.z);
    }

    public static Vec3d getOffsetMotion(Entity entity, double offsetX, double offsetY, double offsetZ, float rotationX, float rotationY) {
        return new Vec3d(offsetZ, offsetY, offsetX).rotateZ(rotationX * 0.017453292F).rotateY(-rotationY * 0.017453292F - 1.5707964F);
    }
