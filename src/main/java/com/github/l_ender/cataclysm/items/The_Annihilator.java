...
public class The_Annihilator extends Item implements ModShieldDisable {

    public The_Annihilator(net.minecraft.item.Item.Settings group) {
        super(group);
    ...
    public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltips, TooltipType flags) {
        tooltips.add(Text.translatable("item.cataclysm.annihilator.desc").formatted(Formatting.DARK_GREEN));
        tooltips.add(Text.translatable("item.cataclysm.annihilator2.desc").formatted(Formatting.DARK_GREEN));
    }
}