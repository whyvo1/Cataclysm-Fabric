...
public class The_Immolator extends Item implements ModShieldDisable, ModEnchantable {
    private final Multimap<EntityAttribute, EntityAttributeModifier> annihilator;

    public The_Immolator(Settings group) {
        super(group);
        ...
        if (p_43396_ instanceof PlayerEntity player) {
            ...
            if (i >= 45) {
                ...
                if (hasSucceeded) {
                    ...
                    p_43395_.playSound(null, player.getX(), player.getY(), player.getZ(), SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.PLAYERS, 1.5f, 1F / (player.getRandom().nextFloat() * 0.4F + 0.8F));
