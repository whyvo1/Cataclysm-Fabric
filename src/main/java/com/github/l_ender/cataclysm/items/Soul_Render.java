...
public class Soul_Render extends Item implements RangeTool {
	...
	public static AttributeModifiersComponent createAttributes() {
		return AttributeModifiersComponent.builder()
				.add(
						EntityAttributes.GENERIC_ATTACK_DAMAGE, new EntityAttributeModifier(BASE_ATTACK_DAMAGE_MODIFIER_ID, 14D, EntityAttributeModifier.Operation.ADD_VALUE), AttributeModifierSlot.MAINHAND
				)
				.add(
						EntityAttributes.GENERIC_ATTACK_SPEED, new EntityAttributeModifier(BASE_ATTACK_SPEED_MODIFIER_ID, -2.9F, EntityAttributeModifier.Operation.ADD_VALUE), AttributeModifierSlot.MAINHAND
				)
				.add(
				...
				.add(
						EntityAttributes.PLAYER_BLOCK_INTERACTION_RANGE, new EntityAttributeModifier(BASE_BLOCK_INTERACTION_RANGE_ID, 2.0F, EntityAttributeModifier.Operation.ADD_VALUE), AttributeModifierSlot.MAINHAND
				)
				.build();
	}
	...
	public void onStoppedUsing(ItemStack stack, World level, LivingEntity livingEntity, int timeLeft) {
		...
		if (livingEntity instanceof PlayerEntity player) {
			int i = this.getMaxUseTime(stack,livingEntity) - timeLeft;
...
//					RenderRushAttachment charge = livingEntity.getData(ModDataAttachments.RENDER_RUSH_ATTACHMENT);
					RenderRushCapability charge = ModCapabilities.getOrCreate(livingEntity, ModCapabilities.RENDER_RUSH_CAPABILITY);
					charge.setRush(true);
					charge.setTimer(t / 2);
					charge.setdamage((float) player.getAttributeValue(EntityAttributes.GENERIC_ATTACK_DAMAGE));
					hasSucceeded = true;
	...
	public void appendTooltip(ItemStack stack, TooltipContext context, List<Text> tooltips, TooltipType flags) {
		tooltips.add(Text.translatable("item.cataclysm.soul_render.desc").formatted(Formatting.DARK_GREEN));
		tooltips.add(Text.translatable("item.cataclysm.soul_render2.desc").formatted(Formatting.DARK_GREEN));
	}
}