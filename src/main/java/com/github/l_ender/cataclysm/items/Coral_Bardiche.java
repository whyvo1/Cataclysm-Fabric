...
public class Coral_Bardiche extends TridentItem implements Vanishable,More_Tool_Attribute {
    public static final int THROW_THRESHOLD_TIME = 10;
    public static final float BASE_DAMAGE = 8.0F;
    public static final float SHOOT_POWER = 2.5F;
    private final Multimap<EntityAttribute, EntityAttributeModifier> defaultModifiers;

    public Coral_Bardiche(Settings p_43381_) {
        super(p_43381_);
        ImmutableMultimap.Builder<EntityAttribute, EntityAttributeModifier> builder = ImmutableMultimap.builder();
        builder.put(EntityAttributes.GENERIC_ATTACK_DAMAGE, new EntityAttributeModifier(ATTACK_DAMAGE_MODIFIER_ID, "Tool modifier", 9.0D, EntityAttributeModifier.Operation.ADDITION));
                ...
                if (j <= 0 || player.isTouchingWaterOrRain()) {
                    if (!p_43395_.isClient) {
                        ...
                        if (j == 0) {
                            ...
                            throwntrident.setVelocity(player, player.getPitch(), player.getYaw(), 0.0F, 2.5F + (float)j * 0.5F, 1.0F);
