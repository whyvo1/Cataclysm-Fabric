...
/**
 * Extra alternatives to vanilla's {@link StructureTerrainAdaptation}, for use with YungJigsawStructure.
...
public abstract class EnhancedTerrainAdaptation {
    ...
    private float computeKernelValue(double xDistance, double yDistance, double zDistance) {
        double squaredDistance = MathHelper.squaredMagnitude(xDistance, yDistance, zDistance);
    ...
    public double computeDensityFactor(
        ...
        if (isInKernelRange(kernelX) && isInKernelRange(kernelY) && isInKernelRange(kernelZ)) {
            ...
            double squaredDistance = MathHelper.squaredMagnitude(xDistance, actualYDistanceToAdjustedBottom, zDistance);
             ...
             */
            double multiplier = -actualYDistanceToAdjustedBottom * MathHelper.inverseSqrt(squaredDistance / 2.0) / 2.0;
