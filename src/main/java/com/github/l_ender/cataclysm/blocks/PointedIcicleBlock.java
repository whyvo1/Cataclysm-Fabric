...
public class PointedIcicleBlock extends Block implements LandingBlock, Waterloggable {
    ...
    public PointedIcicleBlock(AbstractBlock.Settings p_154025_) {
        super(p_154025_);
        this.setDefaultState(this.stateManager.getDefaultState().with(TIP_DIRECTION, Direction.UP).with(THICKNESS, Thickness.TIP).with(WATERLOGGED, Boolean.FALSE));
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext p_154040_) {
        ...
        if (direction1 == null) {
            ...
            return dripstonethickness == null ? null : this.getDefaultState().with(TIP_DIRECTION, direction1).with(THICKNESS, dripstonethickness).with(WATERLOGGED, levelaccessor.getFluidState(blockpos).getFluid() == Fluids.WATER);
        }
    }
    ...
    private static void createDripstone(WorldAccess p_154088_, BlockPos p_154089_, Direction p_154090_, Thickness p_154091_) {
        BlockState blockstate = ModBlocks.POINTED_ICICLE.getDefaultState()
                .with(TIP_DIRECTION, p_154090_)
                .with(THICKNESS, p_154091_)
                .with(WATERLOGGED, p_154088_.getFluidState(p_154089_).getFluid() == Fluids.WATER);
    ...
    private static BlockPos findTip(BlockState p_154131_, WorldAccess p_154132_, BlockPos p_154133_, int p_154134_, boolean p_154135_) {
        if (isTip(p_154131_, p_154135_)) {
            ...
            BiPredicate<BlockPos, BlockState> bipredicate = (p_202023_, p_202024_) -> p_202024_.isOf(ModBlocks.POINTED_ICICLE) && p_202024_.get(TIP_DIRECTION) == direction;
            return findBlockVertical(p_154132_, p_154133_, direction.getDirection(), bipredicate, (p_154168_) -> isTip(p_154168_, p_154135_), p_154134_).orElse(null);
        }
    }
    ...
    private static Optional<BlockPos> findRootBlock(World p_154067_, BlockPos p_154068_, BlockState p_154069_, int p_154070_) {
        ...
        BiPredicate<BlockPos, BlockState> bipredicate = (p_202015_, p_202016_) -> p_202016_.isOf(ModBlocks.POINTED_ICICLE) && p_202016_.get(TIP_DIRECTION) == direction;
        return findBlockVertical(p_154067_, p_154068_, direction.getOpposite().getDirection(), bipredicate, (p_154245_) -> !p_154245_.isOf(ModBlocks.POINTED_ICICLE), p_154070_);
    }
    ...
    public boolean canPathfindThrough(BlockState p_154112_, BlockView p_154113_, BlockPos p_154114_, NavigationType p_154115_) {
        return false;
    }
