...
public class Door_of_Seal_Block extends BlockWithEntity {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World p_152194_, BlockState p_152195_, BlockEntityType<T> p_152196_) {
        return checkType(p_152196_, ModTileentites.DOOR_OF_SEAL, Door_Of_Seal_BlockEntity::tick);
    }
    ...
    public void onPlaced(World level, BlockPos pos, BlockState state, @Nullable LivingEntity entity, ItemStack itemStack) {
        ...
        if (!level.isClient) {
            for (int i = 0; i < 8; i++) {
                ...
                if (abovePos != pos) {
                    level.setBlockState(abovePos, defaultGongPart.with(FACING, state.get(FACING)).with(Door_Of_Seal_Part_Block.PART, Door_Of_Seal_Part.CENTER).with(Door_Of_Seal_Part_Block.Y_OFFSET, i), 3);
                }
    ...
    public static class Door_Of_Seal_Part_Block extends HorizontalFacingBlock {
        ...
        public Door_Of_Seal_Part_Block(Settings properties) {
            super(properties);
