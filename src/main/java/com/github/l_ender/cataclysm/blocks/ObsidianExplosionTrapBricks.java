...
public class ObsidianExplosionTrapBricks extends BlockWithEntity {
    public static final MapCodec<ObsidianExplosionTrapBricks> CODEC = createCodec(ObsidianExplosionTrapBricks::new);
    public static final BooleanProperty LIT = Properties.LIT;

    @Override
    public MapCodec<ObsidianExplosionTrapBricks> getCodec() {
        return CODEC;
    }
    ...
    public static boolean shouldTrigger(Entity entity) {
        if(entity instanceof LivingEntity) {
            if (!entity.getType().isIn(ModTag.TRAP_BLOCK_NOT_DETECTED)) {
    ...
    public <T extends BlockEntity> BlockEntityTicker<T> getTicker(World p_152180_, BlockState p_152181_, BlockEntityType<T> p_152182_) {
        return validateTicker(p_152182_, ModTileentites.OBSIDIAN_EXPLOSION_TRAP_BRICKS, ObsidianExplosionTrapBricks_Block_Entity::commonTick);
    }
