...
public class JigsawManager {
    public static Optional<Structure.StructurePosition> assembleJigsawStructure(
        ...
        Optional<PoolStructurePiece> startPieceOptional = getStartPiece(startPool, startJigsawNameOptional, locatePos, structureManager, worldgenRandom);
    ...
    private static Optional<BlockPos> getPosOfJigsawBlockWithName(
        ...
        try {
            ...
            for (StructureTemplate.StructureBlockInfo jigsawBlockInfo : shuffledJigsawBlocks) {
                Identifier jigsawBlockName = Identifier.tryParse(Objects.requireNonNull(jigsawBlockInfo.nbt()).getString("name"));
            ...
            Cataclysm.LOGGER.error("Encountered unexpected NullPointerException while trying to get jigsaw block with name {} from structure pool element {}", name, structurePoolElement);
            return Optional.empty();
        }
