...
public class CataclysmJigsawManager {
            ...
            if (!predicate.test(chunkgenerator.getBiomeSource().getBiome(BiomeCoords.fromBlock(centerX), BiomeCoords.fromBlock(height), BiomeCoords.fromBlock(centerZ), context.noiseConfig().getMultiNoiseSampler()))) {
                ...
                return Optional.of(new Structure.StructurePosition(new BlockPos(centerX, height, centerZ), (builder) -> {
                    ...
                    if (maxDepth >= 0) {
                        ...
                        String tag = null;
                        if (poolelementstructurepiece.getPoolElement() instanceof CataclysmPoolElement) tag = ((CataclysmPoolElement) poolelementstructurepiece.getPoolElement()).getRandomTag(worldgenrandom);
                        ...
                        list.sort((p1, p2) -> {
                            int i1, i2;
                            i1 = i2 = 0;
                            if (p1.getPoolElement() instanceof CataclysmPoolElement) i1 = ((CataclysmPoolElement) p1.getPoolElement()).genOrder;
                            if (p2.getPoolElement() instanceof CataclysmPoolElement) i2 = ((CataclysmPoolElement) p2.getPoolElement()).genOrder;
    ...
    public record PieceSelection(PieceState pieceState, StructurePoolElement origPiece, StructurePoolElement nextPiece, PoolStructurePiece poolelementstructurepiece, StructureBlockInfo origJigsaw, StructureBlockInfo connectedJigsaw, StructureBlockInfo nextJigsaw, BlockBox nextPieceBoundingBoxPlaced, BlockBox nextPieceInteriorBoundingBox, int l2) {}
    ...
    public static Comparator<Pair<StructureBlockInfo, PieceState>> placeOrderComparator = (p1, p2) -> {
        int p1Order, p2Order;
        try {
            p1Order = poolPlaceOrder.getOrDefault(Objects.requireNonNull(p1.getFirst().nbt()).getString("pool"), 0);
            p2Order = poolPlaceOrder.getOrDefault(Objects.requireNonNull(p2.getFirst().nbt()).getString("pool"), 0);
        }
    ...
    public static class Placer {
        ...
        void tryPlacingChildren(StructureBlockInfo thisPieceJigsawBlock, PieceState pieceState, boolean villageBoundaryAdjust, HeightLimitView heightAccessor, NoiseConfig state) {
            ...
            if (thisPieceJigsawBlock.nbt() != null && thisPieceJigsawBlock.nbt().getString("target").equals(pathJigsawName)) {
            ...
            if (poolOptional.isPresent() && (poolOptional.get().getElementCount() != 0 || Objects.equals(poolResourceLocation, StructurePools.EMPTY.getValue()))) {
                ...
                if (fallbackPoolHolder.value().getElementCount() != 0 || fallbackPoolHolder.matchesKey(StructurePools.EMPTY)) {
                    ...
                    LOGGER.warn("Empty or non-existent fallback pool: {}", fallbackPoolHolder.getKey().map((p_255599_) -> p_255599_.getValue().toString()).orElse("<unregistered>"));
                }
            } else {
        ...
        protected boolean skipJigsawBlock(StructureBlockInfo thisPieceJigsawBlock, PieceState pieceState) {
            if (thisPieceJigsawBlock.nbt() != null && thisPieceJigsawBlock.nbt().getString("target").equals(interiorJigsawName)) {
        ...
        String selectPool(StructureBlockInfo thisPieceJigsawBlock) {
            if (thisPieceJigsawBlock.nbt() != null) {
...
//                if (nextPieceCandidate.toString().contains("room") || nextPieceCandidate.toString().contains("tower")) continue;
                ...
                if (nextPieceCandidate instanceof CataclysmPoolElement CataclysmPoolElement) {
                    ...
                    if (villageBoundaryAdjust && nextPieceBoundingBoxOrigin.getBlockCountY() <= 16) {
                        ...
                        largestSizeOfNextNextPiece = nextPieceJigsawBlocks.stream().mapToInt((blockInfo) -> {
                            ...
                            if (!nextPieceBoundingBoxOrigin.contains(blockInfo.pos().offset(JigsawBlock.getFacing(blockInfo.state())))) {
                                ...
                                Identifier resourcelocation2 = null;
                                if (blockInfo.nbt() != null) {
                                    resourcelocation2 = new Identifier(blockInfo.nbt().getString("pool"));
                                }
                                Optional<StructurePool> optional2 = this.pools.getOrEmpty(resourcelocation2);
                                Optional<StructurePool> optional3 = optional2.flatMap((p_210344_) -> this.pools.getOrEmpty(p_210344_.getFallback().getKey().get()));
                                int j3 = optional2.map((structureTemplatePool) -> structureTemplatePool.getHighestY(this.structureManager)).orElse(0);
                                int k3 = optional3.map((structureTemplatePool) -> structureTemplatePool.getHighestY(this.structureManager)).orElse(0);
                            ...
                            if (checkBounds(nextPieceCandidate, pieceState, nextPieceBoundingBoxPlaced, nextPiecePos, nextPieceRotation, i2)) {
                                ...
                                if (nextPieceCandidate instanceof CataclysmPoolElement cataclysmPoolElement) {
                                    ...
                                    if (needsOverlapBounds.isPresent()) {
                                        if (!specialBounds.getValue().containsKey(needsOverlapBounds.get()) || !VoxelShapes.matchesAnywhere(specialBounds.getValue().get(needsOverlapBounds.get()), VoxelShapes.cuboid(Box.from(nextPieceBoundingBoxPlaced).contract(0.25D)), BooleanBiFunction.AND)) {
                                            continue;
                                        }
                                    }
                                    ...
                                    if (forbiddenOverlapBounds.isPresent()) {
                                        if (specialBounds.getValue().containsKey(forbiddenOverlapBounds.get()) && VoxelShapes.matchesAnywhere(specialBounds.getValue().get(forbiddenOverlapBounds.get()), VoxelShapes.cuboid(Box.from(nextPieceBoundingBoxPlaced).contract(0.25D)), BooleanBiFunction.AND)) {
                                            continue;
                                        }
                                    }
                                }
        ...
        protected boolean checkBounds(StructurePoolElement nextPieceCandidate, PieceState pieceState, BlockBox nextPieceBoundingBoxPlaced, BlockPos nextPiecePos, BlockRotation nextPieceRotation, int i2) {
            ...
            if (nextPieceCandidate instanceof CataclysmPoolElement) {
                ...
                spaceCheckBounds = ((CataclysmPoolElement) nextPieceCandidate).getCheckBoundingBox(this.structureManager, nextPiecePos, nextPieceRotation).offset(0, i2, 0);
            }
            VoxelShape freeSpace = VoxelShapes.combine(free.getValue(), VoxelShapes.cuboid(Box.from(pieceState.piece.getBoundingBox())), BooleanBiFunction.OR);
        ...
        void addNextPieceState(PieceSelection pieceSelection) {
            ...
            if (!(pieceSelection.nextPiece instanceof CataclysmPoolElement && (((CataclysmPoolElement) pieceSelection.nextPiece).ignoresBounds() || !((CataclysmPoolElement) pieceSelection.nextPiece).placeBounds()))) {
                free.setValue(VoxelShapes.combine(free.getValue(), VoxelShapes.cuboid(Box.from(pieceSelection.nextPieceBoundingBoxPlaced)), BooleanBiFunction.ONLY_FIRST));
            }
            ...
            if (pieceSelection.nextPiece instanceof CataclysmPoolElement && pieceSelection.nextPieceInteriorBoundingBox != null) {
                interiorFree.setValue(VoxelShapes.combine(interiorFree.getValue(), VoxelShapes.cuboid(Box.from(pieceSelection.nextPieceInteriorBoundingBox)), BooleanBiFunction.OR));
            }

            // Special bounds - add this piece's bounds to any special bounds its set to
            if (pieceSelection.nextPiece instanceof CataclysmPoolElement cataclysmPoolElement) {
                ...
                if (nextPieceSpecialBounds.isPresent()) {
                    if (specialBounds.getValue().containsKey(nextPieceSpecialBounds.get())) {
                        specialBounds.getValue().put(nextPieceSpecialBounds.get(), VoxelShapes.combine(specialBounds.getValue().get(nextPieceSpecialBounds.get()), VoxelShapes.cuboid(Box.from(pieceSelection.nextPieceBoundingBoxPlaced)), BooleanBiFunction.OR));
                    } else {
                        specialBounds.getValue().put(nextPieceSpecialBounds.get(), VoxelShapes.cuboid(Box.from(pieceSelection.nextPieceBoundingBoxPlaced)));
                    }
                }
            }
            ...
            if (!(pieceSelection.poolelementstructurepiece.getPoolElement() instanceof CataclysmPoolElement) || !((CataclysmPoolElement) pieceSelection.poolelementstructurepiece.getPoolElement()).twoWay()) {
            ...
            if (pieceSelection.nextPiece instanceof CataclysmPoolElement && ((CataclysmPoolElement) pieceSelection.nextPiece).conditions.numPathsOverride.isPresent()) {
                ...
                for (StructureBlockInfo jigsaw : nextJigsaws) {
                    if (jigsaw.nbt() != null && jigsaw.nbt().getString("target").equals(pathJigsawName)) {
                        numPaths++;
                    }
                }
            }
    ...
    static final class InteriorPlacer extends Placer {
        ...
        protected boolean checkBounds(StructurePoolElement nextPieceCandidate, PieceState pieceState, BlockBox nextPieceBoundingBoxPlaced, BlockPos nextPiecePos, BlockRotation nextPieceRotation, int i2) {
            ...
            return ignoreBounds || !VoxelShapes.matchesAnywhere(free.getValue(), VoxelShapes.cuboid(Box.from(spaceCheckBounds).contract(0.25D)), BooleanBiFunction.ONLY_SECOND);
        }
    }
