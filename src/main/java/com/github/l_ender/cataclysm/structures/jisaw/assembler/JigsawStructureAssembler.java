 ...
 * It is originally based on vanilla's Placer class within {@link StructurePoolBasedGenerator}.
 */
public class JigsawStructureAssembler {
     ...
     */
    public void assembleStructure(PoolStructurePiece startPiece, BoxOctree structureBounds) {
     ...
     * This should only be called after {@link JigsawStructureAssembler#assembleStructure(PoolStructurePiece, BoxOctree)}
     * has been called.
     */
    public void addAllPiecesToStructureBuilder(StructurePiecesCollector structurePiecesBuilder) {
    ...
    private void addChildrenForPiece(PieceEntry pieceEntry) {
        ...
        PoolStructurePiece piece = pieceEntry.getPiece();
        MutableObject<BoxOctree> parentOctree = new MutableObject<>();

        // Get list of all jigsaw blocks in this piece
        List<StructureTemplate.StructureBlockInfo> pieceJigsawBlocks = piece.getPoolElement().getStructureBlockInfos(
        ...
        for (StructureTemplate.StructureBlockInfo jigsawBlockInfo : pieceJigsawBlocks) {
            ...
            RegistryKey<StructurePool> poolKey = readPoolName(jigsawBlockInfo);

            // Fetch the target pool, ensuring it's not empty
            Optional<? extends RegistryEntry<StructurePool>> optionalPoolHolder = this.settings.poolRegistry.getEntry(poolKey);
            if (optionalPoolHolder.isEmpty()) {
                Cataclysm.LOGGER.warn("Empty or nonexistent pool: {}", poolKey.getValue());
                continue;
            }
            RegistryEntry<StructurePool> targetPoolHolder = optionalPoolHolder.get();
            StructurePool targetPool = targetPoolHolder.value();
            if (targetPool.getElementCount() == 0 && !targetPoolHolder.matchesKey(StructurePools.EMPTY)) {
                Cataclysm.LOGGER.warn("Empty or nonexistent pool: {}", poolKey.getValue());
                continue;
            }

            // Fetch the jigsaw block's fallback pool (defined in the pool's JSON), ensuring it's not empty
            RegistryEntry<StructurePool> fallbackPoolHolder = targetPoolHolder.value().getFallback();
            StructurePool fallbackPool = fallbackPoolHolder.value();
            if (fallbackPool.getElementCount() == 0 && !fallbackPoolHolder.matchesKey(StructurePools.EMPTY)) {
                Cataclysm.LOGGER.warn("Empty or nonexistent fallback pool: {}", fallbackPoolHolder.getKey()
            ...
            if (pieceEntry.getDepth() != this.settings.maxDepth) {
                pieceContext.candidatePoolElements = new ObjectArrayList<>(targetPool.elementCounts);
            ...
            if (newlyGeneratedPiece.isEmpty()) {
                pieceContext.candidatePoolElements = new ObjectArrayList<>(fallbackPool.elementCounts);
                newlyGeneratedPiece = this.chooseCandidateFromPool(pieceContext);
            }
        ...
        if (pieceEntry.getDeadendPool().isPresent() && !generatedAtLeastOneChildPiece && pieceJigsawBlocks.size() > 1) {
            ...
            Optional<StructurePool> deadendPool = this.settings.poolRegistry.getOrEmpty(deadendPoolId);
            if (deadendPool.isEmpty()) {
                Cataclysm.LOGGER.error("Unable to find deadend pool {} for element {}", deadendPoolId, piece.getPoolElement());
                return;
            }

            PieceEntry parentEntry = pieceEntry.getParentEntry();
            PieceContext newContext = pieceEntry.getSourcePieceContext().copy();
            newContext.candidatePoolElements = new ObjectArrayList<>(deadendPool.get().elementCounts);
            Box pieceAabb = pieceEntry.getPieceAabb();
            if (parentEntry != null && pieceAabb != null) {
                ...
                if (pieceEntry.getPiece().getPoolElement() instanceof CataclysmJigsawPoolElement yungElement
                ...
                if (pieceEntry.getPiece().getPoolElement() instanceof IMaxCountJigsawPoolElement maxCountJigsawPoolElement
    ...
    private PieceContext createPieceContextForJigsawBlock(
        ...
        BlockBox pieceBoundingBox = pieceEntry.getPiece().getBoundingBox();
        ...
        BlockPos jigsawBlockTargetPos = jigsawBlockInfo.pos().offset(direction);
    ...
    private Optional<StructurePoolElement> chooseCandidateFromPool(PieceContext context) {
        ...
        PoolStructurePiece piece = context.pieceEntry.getPiece();
        boolean isPieceRigid = piece.getPoolElement().getProjection() == StructurePool.Projection.RIGID;
        ...
        while (!candidatePoolElements.isEmpty() && totalWeightSum > 0) {
            ...
            for (BlockRotation rotation : BlockRotation.randomRotationOrder(this.settings.rand)) {
                List<StructureTemplate.StructureBlockInfo> candidateJigsawBlocks = chosenPoolElement.getStructureBlockInfos(this.settings.structureTemplateManager, BlockPos.ORIGIN, rotation, this.settings.rand);
                ...
                BlockBox tempCandidateBoundingBox = chosenPoolElement.getBoundingBox(this.settings.structureTemplateManager, BlockPos.ORIGIN, rotation);
                int candidateHeightAdjustments = 0;
                if (this.settings.useExpansionHack && tempCandidateBoundingBox.getBlockCountY() <= 16) {
                    candidateHeightAdjustments = candidateJigsawBlocks.stream().mapToInt((pieceCandidateJigsawBlock) -> {
                        ...
                        RegistryKey<StructurePool> candidateTargetPoolKey = readPoolName(pieceCandidateJigsawBlock);
                        Optional<? extends RegistryEntry<StructurePool>> candidateTargetPool = this.settings.poolRegistry.getEntry(candidateTargetPoolKey);
                        Optional<RegistryEntry<StructurePool>> candidateFallbackPool = candidateTargetPool.map(poolHolder -> poolHolder.value().getFallback());
                        int candidateMaxSize = candidateTargetPool.map(poolHolder -> poolHolder.value().getHighestY(this.settings.structureTemplateManager)).orElse(0);
                        int candidateFallbackMaxSize = candidateFallbackPool.map(poolHolder -> poolHolder.value().getHighestY(this.settings.structureTemplateManager)).orElse(0);
                ...
                for (StructureTemplate.StructureBlockInfo candidateJigsawBlock : candidateJigsawBlocks) {
                    if (!JigsawBlock.attachmentMatches(context.jigsawBlock, candidateJigsawBlock)) continue;
                    ...
                    BlockBox rotatedCandidateBoundingBox = chosenPoolElement.getBoundingBox(this.settings.structureTemplateManager, candidateJigsawBlockRelativePos, rotation);

                    // Determine if candidate is rigid
                    StructurePool.Projection candidateProjection = chosenPoolElement.getProjection();
                    boolean isCandidateRigid = candidateProjection == StructurePool.Projection.RIGID;
                    ...
                    int candidateJigsawYOffsetNeeded = jigsawBlockRelativeY - candidateJigsawBlockRelativeY + JigsawBlock.getFacing(context.jigsawBlock.state()).getOffsetY();
                    ...
                    if (isPieceRigid && isCandidateRigid) {
                        ...
                        if (surfaceHeight == -1) {
                            surfaceHeight = this.settings.chunkGenerator.getHeightOnGround(context.jigsawBlockPos.getX(), context.jigsawBlockPos.getZ(), Heightmap.Type.WORLD_SURFACE_WG, this.settings.levelHeightAccessor, this.settings.randomState);
                        }

                        adjustedCandidatePieceMinY = surfaceHeight - candidateJigsawBlockRelativeY;
                    }
                    int candidatePieceYOffsetNeeded = adjustedCandidatePieceMinY - rotatedCandidateBoundingBox.getMinY();

                    // Offset the candidate's bounding box by the necessary amount
                    BlockBox adjustedCandidateBoundingBox = rotatedCandidateBoundingBox.offset(0, candidatePieceYOffsetNeeded, 0);
                    ...
                    if (candidateHeightAdjustments > 0) {
                        ...
                        adjustedCandidateBoundingBox.maxY = adjustedCandidateBoundingBox.getMinY() + k2;
                    }

                    // Prevent pieces from generating outside the structure's boundaries
                    if (!this.settings.isInBounds(adjustedCandidateBoundingBox.getMaxY()) || !this.settings.isInBounds(adjustedCandidateBoundingBox.getMinY())) {
                        continue;
                    }
                    ...
                    Box aabb = Box.from(adjustedCandidateBoundingBox);
                    Box aabbDeflated = aabb.contract(0.25);
                    ...
                    if (chosenPoolElement instanceof CataclysmJigsawPoolElement yungElement) {
                        StructureContext ctx = new StructureContext.Builder()
                                ...
                                .rotation(rotation)
                                .pieceMinY(adjustedCandidateBoundingBox.getMinY())
                                .pieceMaxY(adjustedCandidateBoundingBox.getMaxY())
                    ...
                    newPiece.addJunction(
                            new JigsawJunction(
                                    ...
                                    -candidateJigsawYOffsetNeeded,
                                    piece.getPoolElement().getProjection())
                    );
    ...
    private void applyModifications() {
        for (PieceEntry pieceEntry : this.pieces) {
            if (pieceEntry.getPiece().getPoolElement() instanceof CataclysmJigsawSinglePoolElement yungElement) {
                if (yungElement.hasModifiers()) {
                    ...
                    StructureContext structureContext = new StructureContext.Builder()
                            ...
                            .pieceEntry(pieceEntry)
                            .pieces(this.pieces)
                            .pieceMaxY(piece.getBoundingBox().getMaxY())
                            .pieceMinY(piece.getBoundingBox().getMinY())
                            .random(this.settings.rand)
                            .randomState(this.settings.randomState)
                            .biomeSource(this.settings.biomeSource)
                            .build();
    ...
    private static RegistryKey<StructurePool> readPoolName(StructureTemplate.StructureBlockInfo jigsawBlockInfo) {
        return RegistryKey.of(RegistryKeys.TEMPLATE_POOL, Identifier.of(jigsawBlockInfo.nbt().getString("pool")));
    }
    ...
    public static class Settings {
         ...
         **/
        private Registry<StructurePool> poolRegistry;
         ...
         **/
        private HeightLimitView levelHeightAccessor;
        ...
        public Settings poolRegistry(Registry<StructurePool> poolRegistry) {
        ...
        public Settings randomState(NoiseConfig randomState) {
        ...
        public Settings levelHeightAccessor(HeightLimitView levelHeightAccessor) {
        ...
        public Settings liquidSettings(StructureLiquidSettings liquidSettings) {
        ...
        public boolean isInBounds(int y) {
            ...
            if (y < levelHeightAccessor.getBottomY() + dimensionPadding.bottom()) return false;
            if (y > levelHeightAccessor.getTopY() - dimensionPadding.top()) return false;
            return true;
        }
    }
}
