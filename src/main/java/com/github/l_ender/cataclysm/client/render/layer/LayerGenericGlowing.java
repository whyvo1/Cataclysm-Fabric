...
public class LayerGenericGlowing<T extends LivingEntity> extends FeatureRenderer<T, EntityModel<T>> {
    private final Identifier texture;
    private final RenderLayer renderType;

    public LayerGenericGlowing(FeatureRendererContext<T, EntityModel<T>> renderer, Identifier texture) {
        super(renderer);
    ...
    public void render(MatrixStack matrixStackIn, VertexConsumerProvider bufferIn, int packedLightIn, T entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        this.getContextModel().render(matrixStackIn, ivertexbuilder, packedLightIn, OverlayTexture.DEFAULT_UV);
