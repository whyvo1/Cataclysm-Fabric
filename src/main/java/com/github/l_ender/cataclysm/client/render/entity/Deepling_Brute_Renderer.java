...
public class Deepling_Brute_Renderer extends MobEntityRenderer<Deepling_Brute_Entity, Deepling_Brute_Model> {
    ...
    @Override
    protected void setupTransforms(Deepling_Brute_Entity p_115317_, MatrixStack p_115318_, float p_115319_, float rotationYaw, float p_115321_, float p_320045_) {
        if (this.isShaking(p_115317_)) {
            rotationYaw += (float)(Math.cos((double)p_115317_.age * 3.25D) * Math.PI * (double)0.4F);
        }

        if (!p_115317_.isInPose(EntityPose.SLEEPING)) {
            p_115318_.multiply(RotationAxis.POSITIVE_Y.rotationDegrees(180.0F - rotationYaw));
        }
        ...
        if (p_115317_.deathTime > 0) {
            ...
            float f1 = direction != null ? getYaw(direction) : rotationYaw;
