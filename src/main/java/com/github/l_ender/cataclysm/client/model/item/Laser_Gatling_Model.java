...
public class Laser_Gatling_Model extends AdvancedEntityModel<Entity> {
    ...
    @Override
    public void setAngles(Entity entity, float openAmount, float switchProgress, float ageInTicks, float netHeadYaw, float headPitch){
        this.resetToDefaultPose();

        this.core.rotationPointY += MathHelper.cos(ageInTicks) * 1F + 1F;
        this.core2.rotationPointY += MathHelper.cos(ageInTicks + (float) Math.PI) * 1F + 1F;


        gatling.rotateAngleZ -= openAmount * 0.75f;
        root.rotationPointZ += MathHelper.cos(openAmount * 2F) * 1F + 1F;
