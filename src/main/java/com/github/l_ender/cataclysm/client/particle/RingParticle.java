...
public class RingParticle extends SpriteBillboardParticle {
    ...
    public RingParticle(ClientWorld world, double x, double y, double z, double motionX, double motionY, double motionZ, float yaw, float pitch, int duration, float r, float g, float b, float opacity, float size, boolean facesCamera, EnumRingBehavior behavior, SpriteProvider sprites) {
        super(world, x, y, z);
    ...
    public static final class RingFactory implements ParticleFactory<RingData> {
        ...
        public Particle createParticle(RingData typeIn, ClientWorld worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new RingParticle(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, typeIn.getYaw(), typeIn.getPitch(), typeIn.getDuration(), typeIn.getR(), typeIn.getG(), typeIn.getB(), typeIn.getA(), typeIn.getScale(), typeIn.getFacesCamera(), typeIn.getBehavior(),spriteSet);
        }
    }

    public static class RingData implements ParticleEffect {
        public static final Factory<RingData> DESERIALIZER = new Factory<>() {
