...
public class SparkTrailParticle extends Particle {
    ...
    public SparkTrailParticle(ClientWorld world, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed, int r, int g, int b) {
        super(world, x, y, z);
    ...
    public void buildGeometry(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        ...
        LightningBoltData.BoltRenderInfo lightningBoltData = new LightningBoltData.BoltRenderInfo(0.5F, 0.1F, 0.5F, 0.85F, new Vector4f(red /255, green /255, blue /255, 1.0F), 0.9F);
        LightningBoltData bolt = new LightningBoltData(lightningBoltData, Vec3d.ZERO, new Vec3d(toX, toY, toZ), 2)
                .size(0.05F)
    ...
    public static class Factory implements ParticleFactory<SparkTrailParticleOptions> {

        @Override
        public Particle createParticle(SparkTrailParticleOptions data, ClientWorld level, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
