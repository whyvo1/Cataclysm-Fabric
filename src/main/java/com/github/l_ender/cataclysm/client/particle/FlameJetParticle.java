...
public class FlameJetParticle extends SpriteBillboardParticle {
    ...
    public void buildGeometry(VertexConsumer buffer, Camera camera, float partialTicks) {
        ...
        buffer.vertex(avector3f[0].x(), avector3f[0].y(), avector3f[0].z()).texture(f7, f5).color(this.red, this.green, this.blue, this.alpha).light(j).next();
        buffer.vertex(avector3f[1].x(), avector3f[1].y(), avector3f[1].z()).texture(f7, f4).color(this.red, this.green, this.blue, this.alpha).light(j).next();
        buffer.vertex(avector3f[2].x(), avector3f[2].y(), avector3f[2].z()).texture(f6, f4).color(this.red, this.green, this.blue, this.alpha).light(j).next();
        buffer.vertex(avector3f[3].x(), avector3f[3].y(), avector3f[3].z()).texture(f6, f5).color(this.red, this.green, this.blue, this.alpha).light(j).next();
...
//        }
    ...
    public static class Factory implements ParticleFactory<DefaultParticleType> {
        ...
        public Particle createParticle(DefaultParticleType typeIn, ClientWorld worldIn, double x, double y, double z, double xSpeed, double ySpeed, double zSpeed) {
            return new FlameJetParticle(worldIn, x, y, z, xSpeed, ySpeed, zSpeed, spriteSet);
        }
    }
}