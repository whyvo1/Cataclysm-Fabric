...
public class Wadjet_Model extends AdvancedEntityModel<Wadjet_Entity> {
	...
	public void setAngles(Wadjet_Entity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		this.chainSwing(tailOriginal, swimSpeed * 4F, swimDegree, -3, limbSwing,limbSwingAmount);
		this.chainSwing(tailOriginal, swimSpeed * 0.6F, swimDegree * 0.15F, -3, ageInTicks,1.0F);
		entity.dc.updateChain(MinecraftClient.getInstance().getTickDelta(), tailOriginal, tailDynamic, 0.4f, 1.5f, 1.8f, 0.87f, 20, true);
	}
