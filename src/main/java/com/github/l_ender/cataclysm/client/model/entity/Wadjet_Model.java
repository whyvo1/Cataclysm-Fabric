...
public class Wadjet_Model extends AdvancedEntityModel<Wadjet_Entity> {
	...
	public void render(MatrixStack matrixStackIn, VertexConsumer bufferIn, int packedLightIn, int packedOverlayIn, int color) {
		this.everything.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, color);
	...
	@Override
	public void setAngles(Wadjet_Entity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		...
		entity.dc.updateChain(MinecraftClient.getInstance().getRenderTickCounter().getTickDelta(true), tailOriginal, tailDynamic, 0.4f, 1.5f, 1.8f, 0.87f, 20, true);
	}
