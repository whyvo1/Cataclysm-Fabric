...
public class AltarOfFire_Block_Entity extends LockableContainerBlockEntity {
    ...
    public AltarOfFire_Block_Entity(BlockPos pos, BlockState state) {
        super(ModTileentites.ALTAR_OF_FIRE, pos, state);
    }

    public static void commonTick(World level, BlockPos pos, BlockState state, AltarOfFire_Block_Entity entity) {
        entity.tick();
    ...
    public void tick() {
        ...
        if (!this.getStack(0).isEmpty()) {
            if(this.getStack(0).getItem() == ModItems.BURNING_ASHES){
                summoningthis = true;
                if(summoningticks == 1) {
                    if (this.world != null) {
                        ScreenShake_Entity.ScreenShake(this.world, Vec3d.ofCenter(this.getPos()), 20, 0.05f, 0, 150);
                    }
                ...
                if(summoningticks > 121) {
                    ...
                    Ignis_Entity ignis = ModEntities.IGNIS.create(world);
                    if (ignis != null) {
                        ignis.setPosition(this.getPos().getX() + 0.5F, this.getPos().getY() + 3, this.getPos().getZ() + 0.5F);
                        ignis.setHomePos(this.getPos());
                        if (world != null && !world.isClient) {
                            world.spawnEntity(ignis);
                        }
                    }
                }
...
//    }
    ...
    protected Text getContainerName() {
        return Text.translatable("block.cataclysm.altar_of_fire");
    }
