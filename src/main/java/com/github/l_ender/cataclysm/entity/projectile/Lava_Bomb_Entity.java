...
public class Lava_Bomb_Entity extends ThrownEntity {
    ...
    protected void initDataTracker(DataTracker.Builder p_326229_) {
        p_326229_.add(ON_GROUND, false);
        p_326229_.add(LAVA_TIME, 0);
        p_326229_.add(MAX_LAVA_TIME, 200);
        p_326229_.add(LAVA_POS, BlockPos.ORIGIN);
    }
    ...
    protected void applyGravity() {
        ...
        if(!this.isOnGround()) {
            if (d0 != (double) 0.0F) {
                this.setVelocity(this.getVelocity().add(0.0F, -d0, 0.0F));
            }
        }else{
            this.setVelocity(this.getVelocity().add(0.0F, 0, 0.0F));
        }
    }
