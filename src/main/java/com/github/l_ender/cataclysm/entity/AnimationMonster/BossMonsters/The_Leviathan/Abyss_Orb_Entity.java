...
public class Abyss_Orb_Entity extends ProjectileEntity {
    ...
    public void tick() {
        ...
        if (this.getWorld().isClient || (entity == null || !entity.isRemoved()) && this.getWorld().isChunkLoaded(this.getBlockPos())) {
            ...
            this.getWorld().addParticle((new LightningParticleOptions(102, 26, 204)), d0 - vec3.x * 0.5D + this.random.nextDouble() * 0.6D - 0.3D, d1 - vec3.y * 0.5D, d2 - vec3.z * 0.5D + this.random.nextDouble() * 0.6D - 0.3D, vec3.x, vec3.y, vec3.z);
