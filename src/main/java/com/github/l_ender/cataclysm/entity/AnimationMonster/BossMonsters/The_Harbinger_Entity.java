...
public class The_Harbinger_Entity extends LLibrary_Boss_Monster implements RangedAttackMob, SkinOverlayOwner {
    ...
    private static final Predicate<LivingEntity> LIVING_ENTITY_SELECTOR = (p_31504_) -> {
        return p_31504_.isMobOrPlayer() && !(p_31504_.getType().isIn(ModTag.TEAM_THE_HARBINGER));
    };
    ...
    protected void initDataTracker(DataTracker.Builder p_326229_) {
        ...
        p_326229_.add(LASER_MODE, false);
        p_326229_.add(ISCHARGE, false);
        p_326229_.add(IS_ACT, true);
        p_326229_.add(FIRST_HEAD_TARGET, 0);
        p_326229_.add(SECOND_HEAD_TARGET, 0);
        p_326229_.add(THIRD_HEAD_TARGET, 0);
        p_326229_.add(OVERLOAD, 0);
    }
    ...
    public void tickMovement() {
        ...
        if (this.getWorld().isClient) {
            if (this.getIsAct()) {
                ...
                double d4 = d1 * dist;
                double d5 = d2 * dist;
                this.getWorld().addParticle(new LightningParticleOptions(255, 51,  0), this.getX() + d0, this.getY() + 2, this.getZ() + d2, d3, d4, d5);
    ...
    private void destoryblock2() {
        ...
        if (this.getAnimation() == NO_ANIMATION) {
            if (!getIsCharge()) {
                if (!getWorld().isClient && this.blockBreakCounter == 0 && this.getWorld().getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
                    ...
                    for (BlockPos pos : BlockPos.iterate(MathHelper.floor(aabb.minX), MathHelper.floor(aabb.minY), MathHelper.floor(aabb.minZ), MathHelper.floor(aabb.maxX), MathHelper.floor(aabb.maxY), MathHelper.floor(aabb.maxZ))) {
                        ...
                        if (!blockstate.isAir() && this.getWorld().getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
        ...
        if(this.getAnimation() != STUN_ANIAMATION) {
            if (!this.getWorld().isClient){
                if (this.destroyBlocksTick > 0) {
                    ...
                    if (this.destroyBlocksTick == 0 && this.getWorld().getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
    ...
    private void performRangedAttack(int head, double targetX, double targetY, double targetZ) {
        ...
        if(this.getIsLaserMode()) {
            ...
            Laser_Beam_Entity laser = new Laser_Beam_Entity(this, vec3.normalize(),this.getWorld(),(float) CMConfig.HarbingerLaserdamage);
            ...
            Wither_Missile_Entity witherskull = new Wither_Missile_Entity(this, vec3.normalize(),this.getWorld(),(float) CMConfig.HarbingerWitherMissiledamage);
            witherskull.setPos(d0, d1, d2);
            this.getWorld().spawnEntity(witherskull);
        }
    }

    public void shootAt(LivingEntity p_31468_, float p_31469_) {
        ...
        public void tick() {
            ...
            if (target != null) {
                ...
                if (entity.getAnimationTick() == 24) {
                    ...
                    //entity.setDeltaMovement(entity.getVelocity().add(vec3.x * 0.8D, 0.9D, vec3.z * 0.8D));
