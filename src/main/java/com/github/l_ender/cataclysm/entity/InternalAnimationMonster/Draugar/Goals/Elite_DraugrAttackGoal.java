...
public class Elite_DraugrAttackGoal extends Goal {
    ...
    public boolean shouldContinue() {
        LivingEntity livingentity = this.mob.getTarget();
        if(this.isHoldingCrossbow()){
          return this.isValidTarget() && (this.canStart() || !this.mob.getNavigation().isIdle());
        }else{
            if (livingentity == null) {
                return false;
            } else if (!livingentity.isAlive()) {
    ...
    public void stop() {
        ...
        if (isHoldingCrossbow()) {
            if (this.mob.isUsingItem()) {
                ...
                this.mob.getActiveItem().set(DataComponentTypes.CHARGED_PROJECTILES, ChargedProjectilesComponent.DEFAULT);
            }
        }
    ...
    public void tick() {
        ...
        if (target != null) {
            double t0 = this.mob.squaredDistanceTo(target);
            if (isHoldingCrossbow()) {
                boolean flag = this.mob.getVisibilityCache().canSee(target);
                ...
                if (flag2) {
                    ...
                    if (this.updatePathDelay <= 0) {
                        this.mob.getNavigation().startMovingTo(target, this.speedModifier * 0.5F);
                ...
                if(t0 < 10){
                    this.mob.equipStack(EquipmentSlot.MAINHAND, new ItemStack(ModItems.BLACK_STEEL_SWORD));
                }
                ...
                this.mob.getLookControl().lookAt(target, 30.0F, 30.0F);
                double d0 = this.mob.squaredDistanceTo(target);
                ...
                if ((this.followingTargetEvenIfNotSeen || this.mob.getVisibilityCache().canSee(target)) && this.ticksUntilNextPathRecalculation <= 0 && (this.pathedTargetX == 0.0D && this.pathedTargetY == 0.0D && this.pathedTargetZ == 0.0D || target.squaredDistanceTo(this.pathedTargetX, this.pathedTargetY, this.pathedTargetZ) >= 1.0D || this.mob.getRandom().nextFloat() < 0.05F)) {
                    ...
                    if (this.canPenalize) {
                        ...
                        if (this.mob.getNavigation().getCurrentPath() != null) {
                            ...
                            if (finalPathPoint != null && target.squaredDistanceTo(finalPathPoint.x, finalPathPoint.y, finalPathPoint.z) < 1)
                ...
                if(t0 > 10){
                    this.mob.equipStack(EquipmentSlot.MAINHAND, new ItemStack(Items.CROSSBOW));
                }
