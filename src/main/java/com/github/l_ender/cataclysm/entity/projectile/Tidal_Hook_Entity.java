...
public class Tidal_Hook_Entity extends PersistentProjectileEntity {
	...
	public Tidal_Hook_Entity(EntityType<? extends PersistentProjectileEntity> type, PlayerEntity owner, World world) {
		super(type, owner, world);
		this.setNoGravity(true);
		this.setDamage(0);
	}

	public Tidal_Hook_Entity(World world, double x, double y, double z) {
		super(ModEntities.TIDAL_HOOK, x, y, z, world);
		this.setNoGravity(true);
		this.setDamage(0);
	}

	public Tidal_Hook_Entity(EntityType<Tidal_Hook_Entity> tidal_hook_entityEntityType, World level) {
	...
	protected void initDataTracker() {
		...
		this.dataTracker.startTracking(HOOKED_ENTITY_ID, 0);
	}
	...
	public void tick() {
		...
		if(getOwner() instanceof PlayerEntity owner) {
			HookCapability hookCapability = ModCapabilities.getOrCreate(owner, ModCapabilities.HOOK_CAPABILITY);
			if (hookCapability != null) {
				...
				if (!getWorld().isClient) {
					if (owner.isDead() || !hookCapability.hasHook() || owner.distanceTo(this) > maxRange || !(owner.getMainHandStack().getItem() instanceof Tidal_Claws || owner.getOffHandStack().getItem() instanceof Tidal_Claws))
						kill();
