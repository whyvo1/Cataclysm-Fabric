...
public class Endermaptera_Entity extends HostileEntity implements IAnimatedEntity {
    @Nullable
    private RegistryKey<LootTable> HAS_JAWS_LOOT = RegistryKey.of(RegistryKeys.LOOT_TABLE, Cataclysm.modIdentifier("entities/endermaptera_has_jaws"));
    ...
    protected void initDataTracker(DataTracker.Builder p_326229_) {
        ...
        p_326229_.add(CLIMBING, (byte) 0);
        p_326229_.add(ATTACHED_FACE, Direction.DOWN);
        p_326229_.add(HAS_JAWS, true);
    }
    ...
    public void tick() {
        ...
        if (!this.getWorld().isClient) {
            ...
            if (this.isOnGround() || this.isInsideWaterOrBubbleColumn() || this.isInLava()) {
                ...
                for (Direction dir : HORIZONTALS) {
                    ...
                    Vec3d offset = Vec3.ofCenter(offsetPos);
        ...
        if (this.getAttachmentFacing() != Direction.DOWN) {
            if (this.getAttachmentFacing() == Direction.UP) {
                ...
                if (!this.horizontalCollision && this.getAttachmentFacing() != Direction.UP) {
                    Vec3d vec = Vec3.of(this.getAttachmentFacing().getVector());
        ...
        if (this.getAttachmentFacing() == Direction.UP) {
            ...
            this.setVelocity(Vec3.multiply(0.7D, 1D, 0.7D));
        } else {
            this.setNoGravity(false);
        }
        if (!flag) {
            if (this.isClimbing()) {
                this.setVelocity(Vec3.multiply(1.0D, 0.4D, 1.0D));
            }
        }
    ...
    public static boolean canSpawn(EntityType<Endermaptera_Entity> entity, ServerWorldAccess worldIn, SpawnReason reason, BlockPos pos, Random randomIn) {
        return !worldIn.getBlockState(pos.down()).isIn(ModTag.ENDERMAPTERA_CAN_NOT_SPAWN) && canSpawnInDark(entity,worldIn,reason,pos,randomIn);
    }
