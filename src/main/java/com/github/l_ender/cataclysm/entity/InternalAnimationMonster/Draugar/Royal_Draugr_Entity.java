...
public class Royal_Draugr_Entity extends HostileEntity implements IShieldEntity {
    ...
    public void damageShield(float p_36383_) {
        if (this.activeItemStack.isIn(ConventionalItemTags.SHIELD_TOOLS)) {
            if (p_36383_ >= 3.0F) {
                int i = 1 + MathHelper.floor(p_36383_);
                Hand interactionhand = this.getActiveHand();
                if (this.getWorld() instanceof ServerWorld serverlevel && !isInCreativeMode()) {
                    Utilities.damageItemStack(this.activeItemStack, i, serverlevel, this, item -> {
                        this.sendEquipmentBreakStatus(item, getSlotForHand(interactionhand));
                        clearActiveItem(); // Neo: Fix MC-168573 ("After breaking a shield, the player's off-hand can't finish using some items")
                    });
                }
    ...
    public boolean isDraugrBlocking() {
        if (this.isUsingItem() && !this.activeItemStack.isEmpty()) {
            ...
            return this.activeItemStack.isIn(ConventionalItemTags.SHIELD_TOOLS) && item.getMaxUseTime(this.activeItemStack, this) - this.itemUseTimeLeft >= 5;
        } else {
            return false;
        }
    ...
    public EntityData initialize(ServerWorldAccess p_34088_, LocalDifficulty p_34089_, SpawnReason p_34090_, @Nullable EntityData p_34091_) {
        ...
        this.updateEnchantments(p_34088_,randomsource, p_34089_);
