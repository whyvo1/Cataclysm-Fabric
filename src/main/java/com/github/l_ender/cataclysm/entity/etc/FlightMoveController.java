        ...
        if (this.state == State.MOVE_TO) {
            ...
            if (d0 < parentEntity.getBoundingBox().getAverageSideLength()) {
                ...
                if (needsYSupport) {
                    ...
                    parentEntity.setVelocity(parentEntity.getVelocity().add(0.0D, (double) parentEntity.getMovementSpeed() * speedGeneral * MathHelper.clamp(d1, -1, 1) * 0.6F, 0.0D));
                }
                if (parentEntity.getTarget() == null || !shouldLookAtTarget) {
                    Vec3d vector3d1 = parentEntity.getVelocity();
                    parentEntity.setYaw(-((float) MathHelper.atan2(vector3d1.x, vector3d1.z)) * (180F / (float) Math.PI));
                    ...
                    parentEntity.setYaw(-((float) MathHelper.atan2(d2, d1)) * (180F / (float) Math.PI));
                    parentEntity.bodyYaw = parentEntity.getYaw();
                }
            }
