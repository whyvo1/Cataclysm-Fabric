    ...
    public void openInventory(PlayerEntity playerEntity) {
        if(playerEntity instanceof ServerPlayerEntity serverplayer) {
            if (isAlive()) {
                ...
                NetworkHandler.sendToPlayer(serverplayer, new MessageOpenInventory(serverplayer.screenHandlerSyncId, this.miniInventory.size(), this.getId()));
...
//                NeoForge.EVENT_BUS.post(new PlayerContainerEvent.Open(serverplayer, serverplayer.currentScreenHandler));
    ...
    public void writeCustomDataToNbt(NbtCompound compound) {
        ...
        for(int i = 1; i < this.miniInventory.size(); ++i) {
    ...
    public void readCustomDataFromNbt(NbtCompound compound) {
        ...
        for(int i = 0; i < listtag.size(); ++i) {
            ...
            if (j < this.miniInventory.size() - 1) {
                this.miniInventory.setStack(j + 1, ItemStack.fromNbt(this.getRegistryManager(), compoundtag).orElse(ItemStack.EMPTY));
            }
        }
    ...
    public ItemStack getBucketItem() {
        ItemStack stack = new ItemStack(ModItems.NETHERITE_MINISTROSITY_BUCKET);
        return stack;
    }
