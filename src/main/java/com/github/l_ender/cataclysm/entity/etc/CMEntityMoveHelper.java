...
public class CMEntityMoveHelper extends MoveControl
{
    ...
    public CMEntityMoveHelper(MobEntity entitylivingIn, float maxRotate)
    {
    ...
    public void tick()
    {
        if (this.state == State.STRAFE)
        {
            ...
            if (pathnavigate != null)
            {
                ...
                if (nodeprocessor != null && nodeprocessor.getDefaultNodeType(this.entity, BlockPos.ofFloored(MathHelper.floor(this.entity.getX() + (double)f7), MathHelper.floor(this.entity.getY()), MathHelper.floor(this.entity.getZ() + (double)f8))) != PathNodeType.WALKABLE) {
