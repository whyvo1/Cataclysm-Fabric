...
public class Abyss_Blast_Entity extends Entity {
    ...
    private static final TrackedData<Float> YAW = DataTracker.registerData(Abyss_Blast_Entity.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Float> PITCH = DataTracker.registerData(Abyss_Blast_Entity.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Integer> DURATION = DataTracker.registerData(Abyss_Blast_Entity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> CASTER = DataTracker.registerData(Abyss_Blast_Entity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Float> BEAMDIRECTION = DataTracker.registerData(Abyss_Blast_Entity.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Float> DAMAGE = DataTracker.registerData(Abyss_Blast_Entity.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Float> HPDAMAGE = DataTracker.registerData(Abyss_Blast_Entity.class, TrackedDataHandlerRegistry.FLOAT);
    ...
    public Abyss_Blast_Entity(EntityType<? extends Abyss_Blast_Entity> type, World world) {
        super(type, world);
    ...
    public Abyss_Blast_Entity(EntityType<? extends Abyss_Blast_Entity> type, World world, LivingEntity caster, double x, double y, double z, float yaw, float pitch, int duration, float direction,float damage,float Hpdamage) {
        this(type, world);
        this.caster = caster;
        this.setYaw(yaw);
        this.setPitch(pitch);
            ...
            if (blockSide != null) {
                ...
                if (!this.getWorld().isClient) {
                    for (BlockPos pos : BlockPos.iterate(MathHelper.floor(collidePosX - 0.5F), MathHelper.floor(collidePosY - 0.5F), MathHelper.floor(collidePosZ - 0.5F), MathHelper.floor(collidePosX + 0.5F), MathHelper.floor(collidePosY + 0.5F), MathHelper.floor(collidePosZ + 0.5F))) {
                        ...
                        if (!block.isAir() && !block.isIn(ModTag.LEVIATHAN_IMMUNE) && this.getWorld().getGameRules().getBoolean(GameRules.DO_MOB_GRIEFING)) {
    ...
    private void spawnExplosionParticles(int amount) {
        for (int i = 0; i < amount; i++) {
            ...
            getWorld().addParticle((new LightningParticleOptions(102, 26, 204)), collidePosX, collidePosY + 0.1, collidePosZ, motionX, motionY, motionZ);
        }
    }
    ...
    protected void initDataTracker(DataTracker.Builder p_326229_) {
        ...
        p_326229_.add(BEAMDIRECTION, 90f);
    ...
    private void updateWithHarbinger() {
        ...
        this.setPosition(caster.getX(),caster.getY() +  1.15f , caster.getZ());
    }
