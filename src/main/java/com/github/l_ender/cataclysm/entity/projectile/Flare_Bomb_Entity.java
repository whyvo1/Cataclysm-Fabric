...
public class Flare_Bomb_Entity extends ThrownEntity {
    ...
    protected void onEntityHit(EntityHitResult result) {
        ...
        if (!this.getWorld().isClient&& !(result.getEntity() instanceof Flare_Bomb_Entity || shooter instanceof Netherite_Monstrosity_Entity && (result.getEntity() instanceof Netherite_Monstrosity_Part || result.getEntity() instanceof Netherite_Monstrosity_Entity)||
            ...
            if (entity1 instanceof LivingEntity livingentity) {
                flag = entity.damage(this.getDamageSources().mobProjectile(this, livingentity), (float) CMConfig.FlareBombDamage);
                if (flag) {
                    if (entity.isAlive()) {
    ...
    public void handleStatus(byte id) {
        ...
        if (id == 4) {
            this.getWorld().playSound(this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_GENERIC_EXPLODE, SoundCategory.BLOCKS, 4.0F, (1.0F + (this.getWorld().random.nextFloat() - this.getWorld().random.nextFloat()) * 0.2F) * 0.7F, false);
