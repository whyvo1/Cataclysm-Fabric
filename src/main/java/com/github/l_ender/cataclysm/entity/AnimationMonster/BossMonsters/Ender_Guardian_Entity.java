...
public class Ender_Guardian_Entity extends LLibrary_Boss_Monster {
    ...
    public static DefaultAttributeContainer.Builder ender_guardian() {
        return HostileEntity.createHostileAttributes()
                ...
                .add(EntityAttributes.GENERIC_STEP_HEIGHT, 1.75F)
    ...
    protected void initDataTracker(DataTracker.Builder p_326229_) {
        ...
        p_326229_.add(IS_HELMETLESS, false);
        p_326229_.add(TELEPORT_POS, Optional.empty());
        p_326229_.add(USED_MASS_DESTRUCTION, false);
    }
    ...
    public void readCustomDataFromNbt(NbtCompound compound) {
        ...
        NbtHelper.toBlockPos(compound, "beam_target").ifPresent(this::setTeleportPos);
    ...
    private void Attackparticle(float vec, float math) {
        if (this.getWorld().isClient) {
            ...
            this.getWorld().addParticle(new RingParticleOptions(0f, (float) Math.PI / 2f, 25, 255, 255, 255, 1f, 25f, false, 2), getX() + vec * vecX + f * math, getY() + 0.3f, getZ() + vec * vecZ + f1 * math, 0, 0, 0);
        }
    }
    ...
    private void BrokenHelmet() {
        if (!this.getWorld().isClient) {
            ...
            this.getWorld().createExplosion(this, this.getX() + xx, this.getY() + (double) this.getStandingEyeHeight(), getZ() + zz, 2.0F, World.ExplosionSourceType.TRIGGER);
            ...
            if (this.getAnimationTick() == 54) {
                    ...
                    ty -= 1;
                {
                    ...
                    for (int i = -4; i <= 4; i++) {
                        ...
                        Vec3d vec3 = new Vec3d(x, this.getY() + 2, z);
                        Ender_Guardian_Bullet_Entity bullet = new Ender_Guardian_Bullet_Entity(getWorld(), this,  vec3.normalize());
...
//            if (event.isCanceled()) return false;
            ...
            boolean flag2 = this.ProperTeleport(p_32544_, p_32545_, p_32546_, true);
            if (flag2) {
