...
//        }
    ...
    protected void initDataTracker() {
        ...
        this.dataTracker.startTracking(TOMBSTONE_POS, BlockPos.ORIGIN);
        this.dataTracker.startTracking(TOMBSTONE_DIRECTION, Direction.NORTH);
...
//    }
    ...
    public void updatePassengerPosition(Entity passenger, Entity.PositionUpdater moveFunc) {
        ...
        double y = this.getY() + passenger.getHeightOffset() + 2.0D;
    ...
    @Nullable
    public EntityData initialize(ServerWorldAccess p_30153_, LocalDifficulty p_30154_, SpawnReason p_30155_, @Nullable EntityData p_30156_, @Nullable NbtCompound p_30157_) {
        this.setTombstonePos(this.getBlockPos());
        this.setTombstoneDirection(Direction.SOUTH);
        return super.initialize(p_30153_, p_30154_, p_30155_, p_30156_, p_30157_);
    }
    ...
    public void tick() {
        ...
        if (!this.getPassengerList().isEmpty() && this.getPassengerList().get(0).isSneaking() && (this.getAttackState() == 31 || this.getAttackState() == 32 || this.getAttackState() == 33 ) ) {
        ...
        if (this.getAttackState() == 7) {
            ...
            if (this.attackTicks == 34) {
                ...
                if (this.getWorld().isClient) {
                    ...
                    double vecX = Math.cos(theta);
                    double vecZ = Math.sin(theta);
                    this.getWorld().addParticle(new RingParticle.RingData(0f, (float) Math.PI / 2f, 30, 0.337f, 0.925f, 0.8f, 1.0f, 85, false, RingParticle.EnumRingBehavior.GROW), getX() + vec * vecX + f * math, getY() + 0.02f, getZ() + vec * vecZ + f1 * math, 0, 0, 0);
                }
    ...
    private void MakeRingparticle(float vec, float math, int duration, float r, float g, float b, float a, float scale) {
        if (this.getWorld().isClient) {
            ...
            double vecX = Math.cos(theta);
            double vecZ = Math.sin(theta);
            this.getWorld().addParticle(new RingParticle.RingData(0f, (float) Math.PI / 2f, duration, r, g, b, a, scale, false, RingParticle.EnumRingBehavior.GROW_THEN_SHRINK), getX() + vec * vecX + f * math, getY() + 0.02f, getZ() + vec * vecZ + f1 * math, 0, 0, 0);
        }
    }
