...
public class Animation_Monsters extends HostileEntity implements Monster {
    ...
    private static final Identifier MOB_HEALTH_MODIFIER_ID = Cataclysm.modIdentifier("animation_health");
    private static final Identifier MOB_DAMAGE_MODIFIER_ID = Cataclysm.modIdentifier("animation_damage");
    ...
    public Animation_Monsters(EntityType entity, World world) {
        super(entity, world);
        if (world.isClient) {
    ...
    public void tick() {
        ...
        if (!getWorld().isClient && getBossMusic() != null) {
            NetworkHandler.sendToPlayersTrackingEntity(this, new MessageMusic(this.getId(), canPlayMusic()));
//            if (canPlayMusic()) {
...
//            }
    ...
    public static void setConfigattribute(LivingEntity entity, double hpconfig, double dmgconfig) {
        ...
        if (maxHealthAttr != null) {
            ...
            maxHealthAttr.addTemporaryModifier(new EntityAttributeModifier(MOB_HEALTH_MODIFIER_ID,  difference, EntityAttributeModifier.Operation.ADD_VALUE));
        ...
        if (attackDamageAttr != null) {
            ...
            attackDamageAttr.addTemporaryModifier(new EntityAttributeModifier(MOB_DAMAGE_MODIFIER_ID,  difference, EntityAttributeModifier.Operation.ADD_VALUE));
        }
    }
...
//        if (net.neoforged.neoforge.common.CommonHooks.onLivingDeath(this, cause)) return;
        if (!this.isRemoved() && !this.dead) {
            ...
            if (this.getWorld() instanceof ServerWorld serverlevel) {
                if (entity == null || entity.onKilledOther(serverlevel, this)) {
                    this.emitGameEvent(GameEvent.ENTITY_DIE);
                    this.drop(serverlevel, cause);
