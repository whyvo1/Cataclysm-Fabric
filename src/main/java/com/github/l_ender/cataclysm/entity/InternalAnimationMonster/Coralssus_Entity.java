...
public class Coralssus_Entity extends Internal_Animation_Monster implements VariantHolder<Coralssus_Entity.Variant>, ISemiAquatic {
    ...
    public Coralssus_Entity(EntityType entity, World world) {
        super(entity, world);
    ...
    protected void initDataTracker(DataTracker.Builder p_326229_) {
        ...
        p_326229_.add(VARIANT, Variant.FIRE.id);
        p_326229_.add(RIGHT, false);
        p_326229_.add(CORALSSUS_SWIM, false);
    }
    ...
    private void Makeparticle(float size,float vec, float math) {
        if (this.getWorld().isClient) {
            ...
            this.getWorld().addParticle(new RingParticleOptions(0f, (float)Math.PI/2f, 30, 255, 255,  255, 1.0f, 20f, false, 2), getX() + vec * vecX + f * math, getY() + 0.2f, getZ() + vec * vecZ + f1 * math, 0, 0, 0);
        }
    }
    ...
    protected void updatePassengerPosition(Entity p_289537_, PositionUpdater p_289541_) {
        ...
        Vec3d vec32 = (new Vec3d(0.0D, 0.0D, f)).rotateY(-this.bodyYaw * ((float)Math.PI / 180F));
        p_289541_.accept(p_289537_, vec3.x - vec31.x  + vec32.x, this.getBodyY(0.75D) , vec3.z - vec31.z + vec32.z);
    }
    ...
    public enum Variant implements StringIdentifiable {
        ...
        private static final IntFunction<Variant> BY_ID = ValueLists.createIdToValueFunction(Variant::id, values(), FIRE);
