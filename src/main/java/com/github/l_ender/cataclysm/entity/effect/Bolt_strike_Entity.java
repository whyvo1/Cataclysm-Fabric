...
public class Bolt_strike_Entity extends Entity {
    ...
    private static final TrackedData<Float> DAMAGE = DataTracker.registerData(Bolt_strike_Entity.class, TrackedDataHandlerRegistry.FLOAT);
    private static final TrackedData<Float> HPDAMAGE = DataTracker.registerData(Bolt_strike_Entity.class, TrackedDataHandlerRegistry.FLOAT);

    private static final TrackedData<Integer> R = DataTracker.registerData(Bolt_strike_Entity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> G = DataTracker.registerData(Bolt_strike_Entity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> B = DataTracker.registerData(Bolt_strike_Entity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    public Bolt_strike_Entity(EntityType<? extends Bolt_strike_Entity> entityType, World level) {
        super(entityType, level);
        this.warmupDelayTicks = 34;
    }

    public Bolt_strike_Entity(World worldIn, double x, double y, double z, float p_i47276_8_, int p_i47276_9_, float damage, LivingEntity casterIn) {
    ...
    public boolean shouldRender(double distance) {
        double d0 = (double)64.0F * getRenderDistanceMultiplier();
    ...
    private void smolder(int amount) {
        for (int i = 0; i < amount; i++) {
            ...
            getWorld().addParticle((new LightningParticleOptions(this.getR(), this.getG(), this.getB())), this.getX() + x, this.getY() + 0.1, this.getZ() + z, motionX, motionY, motionZ);
        }
