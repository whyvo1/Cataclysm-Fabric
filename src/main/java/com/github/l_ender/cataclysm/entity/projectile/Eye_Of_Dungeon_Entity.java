...
public class Eye_Of_Dungeon_Entity extends Entity implements FlyingItemEntity {
    ...
    public void setItem(ItemStack p_32046_) {
        this.getDataTracker().set(DATA_ITEM_STACK, Util.make(p_32046_.copy(), (p_36978_) -> p_36978_.setCount(1)));
    }
    ...
    protected void initDataTracker() {
        ...
        this.getDataTracker().startTracking(R, 0);
        this.getDataTracker().startTracking(G, 0);
        this.getDataTracker().startTracking(B, 0);
    }
    ...
    public void tick() {
        if (this.getStack().isEmpty()) {
            ...
            if (this.isTouchingWater()) {
                ...
                this.getWorld().addParticle((new LightningParticle.OrbData(this.getR(), this.getG(),  this.getB())), d0 - vec3.x * 0.25D + this.random.nextDouble() * 0.6D - 0.3D, d1 - vec3.y * 0.25D - 0.5D, d2 - vec3.z * 0.25D + this.random.nextDouble() * 0.6D - 0.3D, vec3.x, vec3.y, vec3.z);
            }
    ...
    public void writeCustomDataToNbt(NbtCompound p_36975_) {
        ...
        if (!itemstack.isEmpty()) {
            p_36975_.put("Item", itemstack.writeNbt(new NbtCompound()));
        }
    ...
    public void readCustomDataFromNbt(NbtCompound p_36970_) {
        ItemStack itemstack = ItemStack.fromNbt(p_36970_.getCompound("Item"));
