...
public class CMRecipes
{
	...
	public List<WeaponfusionRecipe> getWeaponfusionRecipes() {
		return recipeManager.listAllOfType(ModRecipeTypes.WEAPON_FUSION).stream().toList();
	}

	public List<AltarOfAmethystRecipe> getAmethystBlessRecipes() {
		return recipeManager.listAllOfType(ModRecipeTypes.AMETHYST_BLESS).stream().toList();
	}
